...
export default {
    ...
    methods: {
        ...
        async getJenisAlatById() {
            try {
                let dataToken = localStorage.getItem('token')
                const dataJenisAlat = await axios.get(
                    `${ipBackend}/jenis-alat/detailsById/${this.$route.params.id}`, {
                        headers: {
                            token: dataToken
                        }
                    }
                );
                this.itemJenisAlat = dataJenisAlat.data.data[0];
            } catch (error) {
                console.log(error);
            }
        },
        ...
        async updateJenisAlat() {
            try {
                ...
                this.$swal({
                    icon: "success",
                    title: "Sukses",
                    text: "Data jenis alat berhasil diupdate",
